...
// DELETE /api/cache/snapshots/:index
// Deletes a single snapshot by its position (0-based index) in the sorted set.
// The frontend passes the array index from the snapshots list.
router.delete("/snapshots/:index", protect, admin, async (req, res) => {
  const client = getClient();
  if (!client || !getIsConnected()) {
    return res.status(503).json({ message: "Redis not available" });
  }
  ...
  if (isNaN(idx) || idx < 0) {
    return res.status(400).json({ message: "Invalid snapshot index." });
  }

  try {
    // Fetch all snapshot values from the sorted set (ordered by score = capture time)
    const raw = await client.zRange("lightcache:snapshots", 0, -1);
    ...
    // ZREM removes by value — raw[idx] is the exact JSON string stored
    await client.zRem("lightcache:snapshots", raw[idx]);

    console.log(`[Snapshot] Deleted index ${idx}`);
    res.json({ message: "Snapshot deleted.", deleted_index: idx });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
});
  ...
  try {
    ...
    res.json({
      ...
      table_a: {
        ...
        base_paper_rt_reduction_pct: 63.78,
      },
      table_b: {
        ...
        base_paper_th_increase_pct: 32.84,
      },
      table_c: table_c
        ? {
